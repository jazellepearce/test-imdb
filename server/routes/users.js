...
router.post("/updatePassword", auth, async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    if (errors.password || errors.password_confirm) return res.status(400).json({ error: errors });
...
router.post("/reset/:token", async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    if (errors.password || errors.password_confirm) return res.status(400).json({ errors: errors });
    return res.status(400).json({});
  }
});
  ...
  User.findOne({ username: sanitize(req.body.username) }, (err, user) => {
    ...
    user.comparePassword(sanitize(req.body.password), (err, isMatch) => {
      ...
      user.CheckTokenMail(sanitize(req.body.username), (err, tokenfind) => {
        ...
        if (!tokenfind) return res.json({ loginSuccess: false, message: "check your email" });
